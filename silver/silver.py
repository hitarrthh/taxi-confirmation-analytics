...
def rides_bulk():
    ...
    df = df.withColumn('booking_timestamp',col('booking_timestamp').cast(TimestampType()))
    return df
...
def rides_stream():
    ...
    df_parsed = df.withColumn('parsed_rides',from_json( col('rides'), ride_schema )).select(col('parsed_rides.*'))
    return df_parsed
